...
{
    [Route("api/vehicle_brand")]
    [ApiController]
    public class VBrandController : ControllerBase
    {
        private readonly IvBrand repo;

        public VBrandController(IvBrand repo) {
            this.repo = repo;
        }

        [JwtAdmin]
        [HttpPut("create")]
        public async Task<IActionResult> Create(VBrandModel dto) {
            try {
                if (repo.isBrandExists(dto)) return BadRequest("Brand already exists!");
        ...
        [JwtAdmin]
        [HttpPatch("update")]
        public async Task<IActionResult> Update([FromBody] VBrandModel dto, [FromQuery] int id) {
            try {
                dto.id = id;
                if (repo.isBrandExists(dto)) return BadRequest("Brand already exists!");
